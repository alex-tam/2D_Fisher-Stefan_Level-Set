...
function draw_slices(x, y, U, V, ϕ, i, nx, ny, Lx, Ly)
    gr(); plot() # Load GR plotting backend and clear previous plots
    default(titlefont = (18, "Computer Modern"), guidefont = (26, "Computer Modern"), tickfont = (18, "Computer Modern"))
    plot(x, U[:,ny], xlabel = L"$x$", ylabel = L"$u(x,10,t)$", margin=2mm, legend = false, xlims=(0,Lx))
    savefig("u_slice_x-$i.pdf")
    plot(x, V[:,ny], xlabel = L"$x$", ylabel = L"$V(x,10,t)$", margin=2mm, legend = false, xlims=(0,Lx))
    savefig("V_slice_x-$i.pdf")
    plot(x, ϕ[:,ny], xlabel = L"$x$", ylabel = L"$\phi(x,10,t)$", margin=2mm, legend = false, xlims=(0,Lx))
    savefig("phi_slice_x-$i.pdf")
    plot(y, U[nx,:], xlabel = L"$y$", ylabel = L"$u(10,y,t)$", margin=2mm, legend = false, xlims=(0,Ly))
    savefig("u_slice_y-$i.pdf")
    plot(y, V[nx,:], xlabel = L"$y$", ylabel = L"$V(10,y,t)$", margin=2mm, legend = false, xlims=(0,Ly))
    savefig("V_slice_y-$i.pdf")
    plot(y, ϕ[nx,:], xlabel = L"$y$", ylabel = L"$\phi(10,y,t)$", margin=2mm, legend = false, xlims=(0,Ly))
